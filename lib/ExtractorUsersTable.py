...
from lib.ui.ExtractorUserTable_ui import Ui_UserTable
...
class ExtractorUsersTable(QWidget):
    def __init__(self, main: "MainApp", extractor: "Extractor"):
        try:
            ...
            self.save = True
            self.config = ConfigManager(main, {"users": []}, self.saveDir, f"{self.extractor.name}Users", self)
            #   Always points to self.config.settings["users"]
            self.list = self.config.settings["users"]
            self.extractor.scrollingLabel.setUserListLength(len(self.list))

            #   Keep track of hidden rows that will get deleted
            self.removeOnSave = []
            ...
            self.ui.btn_removeUser.clicked.connect(self.hideCheckedRows)
    ...
    def closeEvent(self, event):
        if self.save and self.isTableValid(self.table):
            self.removeHiddenRows()
            self.saveTable()
        else:
            self.showHiddenRows()
    ...
    def setupShortcuts(self):
        ...
        #   Modify users
        QShortcut(QKeySequence("Del"), self).activated.connect(self.hideCheckedRows)
    ...
    def _refreshAllRowStyles(self) -> None:
    ...
    def _saveRowData(self, row) -> dict[Any, Any]:
        data = {}
    ...
    def insertRowFromData(self, data, row) -> None:
        try:
    ...
    def hideCheckedRows(self):
        """Hide checked rows, that then will be either removed or re-shown"""
        rows = self.getCheckedRows()
        if not rows:
            return

        for row in reversed(rows):
            self.removeOnSave.append(row)
            self.table.hideRow(row)

    def showHiddenRows(self):
        """Show hidden rows again if the user pressed discard"""
        for row in self.removeOnSave:
            self.table.showRow(row)
        self.removeOnSave.clear()

    def removeHiddenRows(self):
        """Remove all rows if the user pressed save and there were removed rows"""
        for row in self.removeOnSave:
    ...
    def _updateRowStyle(self, table, row, checked) -> None:
        try:
    ...
    def _updateCheckbox(self, table, row, column, checked=False) -> QCheckBox:
        #   Sortvalue
        ...
        checkbox: QCheckBox = container.findChild(QCheckBox)
        checkbox.setChecked(checked)
        return checkbox

    def _insertCombobox(self, table, row, column, value=0) -> QComboBox:
        try:
            #   Combobox
            combo_box = QComboBox()

            combo_box.addItems(self.getComboTemplate())
    ...
    def _updateCombobox(self, table, row, column, value=0) -> QComboBox:
        #   Combobox
        comboBox: QComboBox = table.cellWidget(row, column)
        comboBox.setCurrentIndex(value)

        return comboBox

    def _insertText(self, table: QTableWidget, row: int, column: int, value: str, editable=False, short=False) -> QTableWidgetItem:
        try:
    ...
    def _updateText(self, table: QTableWidget, row: int, column: int, value: str) -> QTableWidgetItem | None:
    ...
    def getTableData(self) -> dict | None:
        """Returns the table data in json format

        Returns:
            dict with table data
            None if the table is invalid
        """
        try:
            fullTable = []
            ...
            for row in range(self.table.rowCount()):
                ...
                if username in usersSeen:
                    self.main.qtHelper.Throw(
                        f"Duplicate user found: {username}", title="Duplicate User", type=MessageType.WARNING, parent=self
                    )
                    return None
    ...
    def isTableValid(self, table) -> bool:
    ...
    def saveTable(self) -> bool:
        try:
            ...
            self.extractor.scrollingLabel.setUserListLength(len(fullTable["users"]))
            return True
    ...
    def setUserKey(self, username, keyname, value):
        try:
            for i, user in enumerate(self.list):
                self.main.debuggy(f"Current list: {self.list}", self)
                if user.get("UserHandle") == username:
                    ...
                    self.inv(lambda: self.logger.debug(f"setUserKey -> {username}[{keyname}] = '{str(value)[:12]}'..."))

                    #   Test fix 2
                    user[keyname] = value
                    copy = user.copy()

                    self.inv(lambda u=copy, index=i: self.updateRowFromData(u, index))
                    return
    ...
    def getComboTemplate(self):
        t = self.comboTemplate[self.comboIndex % len(self.comboTemplate)]
        self.comboIndex += 1
        return t
