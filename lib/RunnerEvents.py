...
class RunnerEvents:
    ...
    @staticmethod
    def updateTimestamp(debuggy, main, extractor, message):
        try:
            #   Since yt-dlp doesn't let it's interal python process to be called
            #   We have to rely on it's output, so we have to parse the message and thank it for doing so

            extractor.logger.debug("[debug] As you please, Yt-dlp stdout sir.")

            match = re.search(r"Archiver, please timestamp\s+'([^']+)'\s+to\s+'(\d+)'", message)
            if not match:
                debuggy("[debug] Could not find video path in message", "Events")
                return
            ...
            if not os.path.exists(path):
                debuggy("[debug] File does not exist", "Events")
                return

            os.utime(path, (timestamp, timestamp))

            debuggy(f"[debug] {path=} updated to {timestamp=}", "Events")
