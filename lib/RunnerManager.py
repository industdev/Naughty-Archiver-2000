...
import psutil
from lib.RunnerHandler import RunnerHandler


class RunnerManager:
    def __init__(self, main: "MainApp", extractor: "Extractor", runner: "RunnerInterface"):
        ...
        self.runner = runner
        self.lineChanger = RunnerHandler(
            main,
            self,
            self.main.General.config.settings["errorboxes"],
            errorLogger,
            self.main.General.config.settings["stoptrigger"],
            self.runner,
        )

        self.runnerPrefix = runner.getPrefix()
    ...
    def _processOutput(self, stream):
        try:
            ...
            for raw in iter(stream.readline, b""):
                ...
                #   Skip unwanted lines
                if any(re.match(pattern, line) for pattern in self.skipLines):
                    self.main.debuggy(f"Skipped line {line}", self)
                    continue
    ...
    def __flush(self, batch):
        ...
        def log():
            for line, level in batch:
                self.extractor.logger.log(f"[{self.runnerPrefix}]{line}", level)
        ...
        try:
            ...
            if code != 0:
                if code == 3221225595:
                    ...
                    self.stopReason = "General runner error"
                elif code == 126:
                    ...
                    self.stopReason = "Runner process terminated by user"

                msg = self.stopReason or Return.ERR_RUNNER_CODE1
                return False, msg
