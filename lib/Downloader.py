...
from lib.ui.Downloader_ui import Ui_Form
...
class Downloader(QWidget):
    def __init__(self, main: "MainApp", safeTrash: Callable, toolsPath: str, General: "GeneralTab"):
        super().__init__(None)
        self.ui = Ui_Form()
        ...
        self.checkboxes = [self.ui.cfg_gdl, self.ui.cfg_ffmpeg, self.ui.cfg_mkvmerge, self.ui.cfg_ytdlp, self.ui.cfg_aria2c]
    ...
    def getToolsData(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "renameTargets": ["ffmpeg.exe", "ffprobe.exe"],
            ...
            },
            {
                ...
                "targets": ["yt-dlp.exe"],
                "renameTargets": ["yt-dlp.exe"],
                "filename": "yt-dlp.exe",
                "version": [20250905, "2025.09.05"],
                "sha256": "627d0d906d07e63beff5e27a6f15a32d021618b5309e1ef083ffabef771afe93",
                "unzip": False,
            },
            {
                "name": "aria2c",
                ...
                "targets": ["aria2c.exe"],
                "renameTargets": ["aria2c.exe"],
                "filename": "tmp_aria2c.zip",
                "version": [13700, "1.37.0"],
    ...
    def setupVersionLabels(self):
        ...
        labels = [(getattr(self.ui, f"cfg_c{i + 1}"), getattr(self.ui, f"cfg_l{i + 1}")) for i in range(len(tools))]
    ...
    def download(self, url, dest, filename):
        ...
        try:
            ...
            if size:
                ...
                self.inv(lambda: self.loading.start(totalKbSize, f"Downloading {filename}", 100, "KB", 50))
            else:
                self.inv(lambda: self.loading.start(1000, f"Downloading {filename}", 100, "chunks", 50))
