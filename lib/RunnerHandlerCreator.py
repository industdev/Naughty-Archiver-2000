...
from lib.ui.RunnerHandlerCreator import Ui_Creator
...
class RunnerHandlerCreator(QWidget):
    def __init__(self, main: "MainApp", general: "GeneralTab") -> None:
        super().__init__(None)
        start = time.perf_counter()
        self.main = main
        self.inv = main._inv
        self.unlockDefaultEntries = True
    ...
    def saveJson(self):
        with open(self.main.userOutputPatternsFPath, "w", encoding="utf-8") as f:
            ...
            os.fsync(f.fileno())
        self.main.debuggy(f"runnerHandlerCreator::saveJson -> {self.main.userOutputPatternsFPath}", self)

    def compile(self, toCompile=None, classe=None) -> list[tuple[re.Pattern[str], dict[str, Any]]]:
        try:
            classe = self if not classe else classe

            if classe and toCompile:
                patterns = toCompile
            else:
                patterns = self.patterns.copy()

            cachedResult = self.main.cacheManager.serveCache(patterns)
            if cachedResult is not None:
                self.main.debuggy("Compiled patterns from cache", classe)
                return cachedResult

            self.main.debuggy(f"Compiling output handling patterns", classe)

            #   Create the tuple with the compiled pattern and the config dict
            compiled = []
            for config in patterns:
                try:
                    cTuple = (re.compile(config[Handlers.MATCH.name], flags=re.IGNORECASE), config)
                except Exception as e:
                    self.main.varHelper.exception(e)
                    self.main.General.logger.error(f"Error compiling regex pattern: {config[Handlers.MATCH.name]} -> {e}")

                self.main.debuggy(f"-> {cTuple}", classe)
                compiled.append(cTuple)

            self.main.cacheManager.saveCache(patterns, compiled)
            return compiled
        except Exception as e:
            self.main.varHelper.exception(e)
            self.main.qtHelper.Throw(f"Error compiling output handler patterns: {e}", type=MessageType.CRITICAL)
            raise
    ...
    def loadData(self):
        ...
        for i, entry in enumerate(self.patterns):
            ...
            _statemask = entry.get(Handlers.STATEMASK.name)
            _runnermask = entry.get(Handlers.RUNNERMASK.name)
            _isProtected = entry.get("DEFAULT")
            self.addListWidgetEntry(displayText, _level, _statemask, _runnermask, _isProtected)
    ...
    def initializeEmptyState(self):
        ...
        self.ui.cfg_repeatedAction.setCurrentIndex(0)
        self.ui.cfg_runEvent.setCurrentIndex(0)
        self.ui.cfg_mask.setCurrentIndex(0)
        self.ui.cfg_runnerMask.setCurrentIndex(0)
    ...
    def updateToNewVersion(self):
        ...
        #   Since i'm lazy i will make my program write protected for me
        isProtected = False if not defaultEntry.get("DEFAULT") else True
        if not isProtected:
            for entry in self.defaultPatterns:
                entry["DEFAULT"] = True
        ...
        self.main.debuggy(f"{currentDefaultVersion=}", self)
        self.main.debuggy(f"{defaultEntry.get('DEFAULT')=}", self)
        self.main.debuggy(f"{currentAppVersion=}", self)
    ...
    def loadJson(self):
        try:
            ...
            with open(self.main.userOutputPatternsFPath, "r", encoding="utf-8") as f:
                ...
                self.main.debuggy(f"runnerHandlerCreator::loadJson -> {self.main.userOutputPatternsFPath}", self)

            with open(self.main.defaultOutputPatternsFPath, "r", encoding="utf-8") as f:
                self.defaultPatterns = json.load(f)
                self.main.debuggy(f"runnerHandlerCreator::loadJson -> {self.main.defaultOutputPatternsFPath}", self)
            ...
            self.main.cmd.info(
                f"[{datetime.now()}] runnerHandlerCreator::loadJson -> File not found, loading default: {self.main.defaultOutputPatternsFPath}"
            )
            with open(self.main.defaultOutputPatternsFPath, "r", encoding="utf-8") as f:
    ...
    def setupConnections(self):
        ...
        self.ui.cfg_repeatedAction.currentTextChanged.connect(lambda: self.markModified())
        ...
        self.ui.cfg_runnerMask.currentTextChanged.connect(lambda: self.markModified())
    ...
    def populateComboBoxes(self):
        ...
        #   Repeated action combo
        self.ui.cfg_repeatedAction.clear()
        self.ui.cfg_repeatedAction.addItem("Do the same")
        for action in HandlersActions:
            enum_name = f"{action.name}"
            self.ui.cfg_repeatedAction.addItem(action.desc, enum_name)
        ...
        #   statemask combo - store enum name as string, display description
        self.ui.cfg_mask.clear()
        self.ui.cfg_mask.addItem("running", None)
        ...
        #   runnermask combo - store enum name as string, display description
        self.ui.cfg_runnerMask.clear()
        self.ui.cfg_runnerMask.addItem("any runner", None)
        self.ui.cfg_runnerMask.addItem("YT-dlp", "yt-dlp")
        self.ui.cfg_runnerMask.addItem("Gallery-dl", "g-dl")
    ...
    def saveCurrentEntry(self):
        ...
        #   Repeated action
        rActionData = self.ui.cfg_repeatedAction.currentData()
        if rActionData:
            config[Handlers.REPEATED_ACTION.name] = rActionData
        ...
        #   Statemask
        smaskData = self.ui.cfg_mask.currentData()
        if smaskData:
            config[Handlers.STATEMASK.name] = smaskData

        #   Runnermask
        rmaskData = self.ui.cfg_runnerMask.currentData()
        if rmaskData:
            config[Handlers.RUNNERMASK.name] = rmaskData
        ...
        if item and matchText:
            ...
            if rmaskData == "yt-dlp":
                matchText = f"(yt-dlp) {matchText}"
            elif rmaskData == "g-dl":
                matchText = f"(g-dl) {matchText}"
    ...
    def loadEntry(self, index):
        ...
        if action:
            ...
            self.ui.cfg_actionAfterLinesSeen.setValue(0)

        #   Load repeated action
        rAction = config.get(Handlers.REPEATED_ACTION.name)
        if rAction:
            for i in range(self.ui.cfg_repeatedAction.count()):
                if self.ui.cfg_repeatedAction.itemData(i) == rAction:
                    self.ui.cfg_repeatedAction.setCurrentIndex(i)
                    break
        else:
            self.ui.cfg_repeatedAction.setCurrentIndex(0)  # "Do the same"
        ...
        if runnermask:
            for i in range(self.ui.cfg_runnerMask.count()):
                data = self.ui.cfg_runnerMask.itemData(i)
                if data == runnermask:
                    self.ui.cfg_runnerMask.setCurrentIndex(i)
                    break
        else:
            self.ui.cfg_runnerMask.setCurrentIndex(0)  # "Any runner"
    ...
    def blockSignals(self, block):
        widgets = [
            ...
            self.ui.cfg_action,
            self.ui.cfg_repeatedAction,
            self.ui.cfg_runEvent,
            self.ui.cfg_mask,
            self.ui.cfg_runnerMask,
    ...
    def addNewEntry(self):
        ...
        _statemask = newEntry.get(Handlers.STATEMASK.name)
        _runnermask = newEntry.get(Handlers.RUNNERMASK.name)
        self.addListWidgetEntry("Match Me", _level, _statemask, _runnermask, False, index=insertIndex)
    ...
    def duplicateSelectedEntry(self):
        ...
        _statemask = duplicateEntry.get(Handlers.STATEMASK.name)
        _runnermask = duplicateEntry.get(Handlers.RUNNERMASK.name)
        self.addListWidgetEntry(displayText, _level, _statemask, _runnermask, False)
    ...
    def loadJsonData(self, data):
        ...
        for entry in self.patterns:
            ...
            self.addListWidgetEntry(displayText, _level, _statemask, _runnermask, _protected)
    ...
    def addListWidgetEntry(self, text: str, levelValue: str, stateMask, runnerMask, isProtected: bool, index=None):
        string = text

        if runnerMask == "yt-dlp":
            string = f"(yt-dlp) {string}"
        elif runnerMask == "g-dl":
            string = f"(g-dl) {string}"
        ...
        if isProtected:
            color = self.colors.get("PROTECTED")
            item.setForeground(QBrush(color))
