...
from lib.ui.ExtractorCookieEdit_ui import Ui_Form
import os
import subprocess
from PySide6.QtGui import QKeySequence, QShortcut


class ExtractorCookieEdit(QWidget):
    def __init__(self, cookiesDPath: str, galleryFPath: str, extractor: "Extractor"):
        super().__init__(None)
        self.ui = Ui_Form()
        ...
        self.cookiesDPath = cookiesDPath
    ...
    def saveSelection(self):
        cookieName = self.ui.cb_fileDropdown.currentText()
        if cookieName == "":
            return
        ...
        text = self.extractor.config._pathToCookiesFilename(cookieName)
    ...
    def updateComboBox(self):
        ...
        if not os.path.exists(self.cookiesDPath):
            os.makedirs(self.cookiesDPath)
            return

        #   Get all files from the directory that don't end with _token
        files = [
            f for f in os.listdir(self.cookiesDPath) if os.path.isfile(os.path.join(self.cookiesDPath, f)) and not f.endswith("_token")
        ]
    ...
    def loadFile(self):
        ...
        #   Load cookie file
        cookieFilePath = os.path.join(self.cookiesDPath, currentFile)
        ...
        tokenFilePath = os.path.join(self.cookiesDPath, f"{currentFile}_token")
    ...
    def addFile(self):
        ...
        cookieFilePath = os.path.join(self.cookiesDPath, filename)
        tokenFilePath = os.path.join(self.cookiesDPath, f"{filename}_token")
    ...
    def removeFile(self):
        ...
        cookieFilePath = os.path.join(self.cookiesDPath, selected)
        tokenFilePath = os.path.join(self.cookiesDPath, f"{selected}_token")
